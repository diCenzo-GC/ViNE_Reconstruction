...
%% Bacteroid III reactions Classification (after adding bacteroid biomass sink) based on R.A.
...
BacIIIRxnsPos = strfind(model.rxns, 'BacteroidIII'); % find reactions whose name contains the string 'BacteroidIII'
BacIIIRxnsPos(cellfun('isempty',BacIIIRxnsPos))={0}; % replace every empty cell with a zero
BacIIIRxnsPos = cell2mat(BacIIIRxnsPos); % transform cell array into double array
BacIIIRxns = model.rxns(BacIIIRxnsPos==1); % list Bacteroid III reactions
nRxns = length(BacIIIRxns); % number of BacteroidIII reactions
...
for i=1:nRxns
    RxnStr = BacIIIRxns{i}; % reaction specified by the i-th position of BacIIIRxns
    disp(RxnStr);
...
figure
bar(CountOfRxns)
title('Frequencies of Reaction Classes in Bacteroids of Nodule Zone III')
axis = gca;
