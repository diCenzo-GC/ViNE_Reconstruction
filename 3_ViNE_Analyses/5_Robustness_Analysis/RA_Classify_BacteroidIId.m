...
%% Bacteroid IId reactions Classification (after adding bacteroid biomass sink) based on R.A.
...
BacIIdRxnsPos = strfind(model.rxns, 'BacteroidIId'); % find reactions whose name contains the string 'BacteroidIId'
BacIIdRxnsPos(cellfun('isempty',BacIIdRxnsPos))={0}; % replace every empty cell with a zero
BacIIdRxnsPos = cell2mat(BacIIdRxnsPos); % transform cell array into double array
BacIIdRxns = model.rxns(BacIIdRxnsPos==1); % list Bacteroid IId reactions
nRxns = length(BacIIdRxns); % number of BacteroidIId reactions
...
for i=1:nRxns
    RxnStr = BacIIdRxns{i}; % reaction specified by the i-th position of BacIIdRxns
    disp(RxnStr);
...
figure
bar(CountOfRxns)
title('Frequencies of Reaction Classes in Bacteroids of Nodule Zone IId')
axis = gca;
