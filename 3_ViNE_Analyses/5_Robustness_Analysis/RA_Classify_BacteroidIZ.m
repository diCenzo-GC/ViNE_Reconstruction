...
%% Bacteroid IZ reactions Classification (after adding bacteroid biomass sink) based on R.A.
....................
...
BacIZRxnsPos = strfind(model.rxns, 'BacteroidIZ'); % find reactions whose name contains the string 'BacteroidIZ'
BacIZRxnsPos(cellfun('isempty',BacIZRxnsPos))={0}; % replace every empty cell with a zero
BacIZRxnsPos = cell2mat(BacIZRxnsPos); % transform cell array into double array
BacIZRxns = model.rxns(BacIZRxnsPos==1); % list Bacteroid IZ reactions
nRxns = length(BacIZRxns); % number of BacteroidIZ reactions
...
for i=1:nRxns
    RxnStr = BacIZRxns{i}; % reaction specified by the i-th position of BacIZRxns
    disp(RxnStr);
...
figure
bar(CountOfRxns)
title('Frequencies of Reaction Classes in Bacteroids of Nodule Zone IZ')
axis = gca;
